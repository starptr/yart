...
export default function Icecamp(props: IcecampProps) {
  ...
  return <Layout>
    ...
    <h1 style={{ textAlign: "center" }} className={`${utilStyles.heading2Xl} ${utilStyles.bold}`}> Icecamp</h1>
    <label>Name: </label>
    <input type="text" value={nameInput} onChange={e => setNameInput(e.target.value)} onKeyDown={e => {
      if (e.key === 'Enter') {
        enterName(nameInput);
      }
    }} />
    ...
    <ul>
      {names.map((name, idx) => <Person key={idx} onClick={_e => removeName(idx)}>{name}</Person>)}
    </ul>
    <input type="button" value="Roll Icecube" disabled={names.length === 0} onClick={handleRoll} />
    ...
    <div style={{ marginTop: "2em" }}>
      <details>
        ...
        <p>This is a tool to do <a href="https://ja.wikipedia.org/wiki/%E4%BA%BA%E5%BF%97%E6%9D%BE%E6%9C%AC%E3%81%AE%E3%81%99%E3%81%B9%E3%82%89%E3%81%AA%E3%81%84%E8%A9%B1">this Japanese TV show</a> in real life.</p>
      </details>
    </div>
  </Layout>;
}
